...
//import com.izenesoft.b5m.common.dto.PageSplitDto;
/**
 * 用于分页工具
 * 
 * @author leo
...
public class PageSpliter {
	 ...
	 * @throws AppException
	 */
	public static final void splitPage(PageSplitDto dto) throws Exception {
		int spNo = 4;
		try {
			/*
			 * 取得总页数
			...
			try {
				countPageNumber = PageSpliter.countPages(dto
						.getCountRecNumber(), dto.getPageSize());
			} catch (Exception e) {
				countPageNumber = 1;
			}
			...
			if (currentPageNo < nextPageNo && currentPageNo < countPageNumber) {
				dto.setHasNextPage("YES");
			} else {
				dto.setHasNextPage(null);
			}
			/*
			 * 计算后一页的页码
			 */
			...
			// 是否有前一页
			if (currentPageNo > 1) {
				dto.setHasPrePage("YES");
			} else {
				dto.setHasPrePage(null);
			}
			/*
			 * 计算前一页的页号(面前有4页)
			 */
			...
			e.printStackTrace();
			// 查询结果时出错
			throw new Exception("err.groupon.001");
		}
	}
