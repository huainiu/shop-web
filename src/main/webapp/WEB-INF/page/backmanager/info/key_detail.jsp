...
<body>
	...
	<script type="text/javascript">
		$('#key-manager').addClass('cur');
		$(function() {
			//返回
			$("#backManage").click(function() {
				var url = "${basePath}backmanager/key/keys.htm" + $.query.REMOVE("id");
				window.location.href = url;
			});
			//提交
			$("#submit").click(function() {
				var id = $("#id").val();
				var url = "${basePath}backmanager/key/save.htm";
				var name = $("#name").val();
				if (!name) {
					alert("名称不能为空");
					return;
				...
				$.ajax({
					...
					success : function(result) {
						alert(result.msg);
						location.href = "${basePath}backmanager/key/edit.htm";
					}
				});
			});
		});
