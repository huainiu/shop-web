...
<script type="text/javascript">
	function gotoPage(currentPage){
		var url = "${basePath}backmanager/key/keys.htm" + $.query.set('currentPage', currentPage).toString();
...
<body>
						...
						<table width="100%" class="user-table">
							<tr>
								...
								<td style="text-align: right;">
									<input type="button" value="添加新关键词" onclick="javascript:window.location.href='${basePath}backmanager/key/edit.htm'+$.query.set('currentPage', ${pageView.currentPage}).toString();" id="add" name="add" class="button" style="margin-right: 10px;">
								</td>
							</tr>
						</table>
						<table width="100%" class="user-table">
							...
							<tbody>
								<c:forEach items="${pageView.records}" var="bean">
									<tr id="${bean.id}">
										...
										<td>${bean.name }</td>
										<td>
											<input value="编辑" class="button" type="button"	onclick="javascript:window.location.href='${basePath}backmanager/key/edit.htm' + $.query.set('id', ${bean.id }).toString();">
	...
	<script type="text/javascript">
		...
		function search(keyword) {
			var url = "${basePath}backmanager/key/keys.htm?keyword="+keyword;
			window.location.href = url;
		}
		
		function delBean(id) {
			var url = "${basePath}backmanager/key/delete.htm";
			$.ajax({
				url : url,
				type : 'POST',
				data : {'id' : id},
